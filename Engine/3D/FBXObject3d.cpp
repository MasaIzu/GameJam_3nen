...
void FBXObject3d::Draw(const WorldTransform& worldTransform, const ViewProjection& viewProjection)
{
	// モデルの割り当てがなければ描画しない
	if (fbxmodel == nullptr) {
		return;
	...
	// 定数バッファビューをセット
	DirectXCore::GetInstance()->GetCommandList()->SetGraphicsRootConstantBufferView(4, constBuffSkin->GetGPUVirtualAddress());
	// モデル描画
	fbxmodel->Draw(DirectXCore::GetInstance()->GetCommandList());
}
...
ViewProjection FBXObject3d::GetCamera()
{
	return *this->camera;
}
